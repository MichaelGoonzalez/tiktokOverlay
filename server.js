...
function connectTikTok() {
  ...
  connection.emit = function (event, ...args) {
    console.log("[TikTok:evento]", event);
  ...
  connection
    ...
    .then((state) =>
      console.log(
        `[TikTok] Conectado a ${TIKTOK_USER} - roomId: ${state.roomId}`,
      ),
    )
    .catch((err) => {
  ...
  connection.on("like", (data) => {
    ...
    bumpCombo();
    const dmg = dealDamage(n);
    io.emit("event", {
      type: "like",
      user: data.user?.nickname || "alguien",
      count: n,
      damage: dmg,
    });
    io.emit("state", sanitize(gameState));
  ...
  connection.on("chat", (data) => {
    //console.log('[TikTok:chat] linea 152:', JSON.stringify(data, null, 2));
    gameState.comments++;
    bumpCombo();
  ...
  connection.on("gift", (data) => {
    //console.log('[TikTok:gift] linea 167:', JSON.stringify(data, null, 2));
  ...
  connection.on("follow", (data) => {
    //console.log('[TikTok:follow] linea 183:', JSON.stringify(data, null, 2));
  ...
  connection.on("share", (data) => {
    //console.log('[TikTok:share] linea 191:', JSON.stringify(data, null, 2));
  ...
  connection.on("roomUser", (data) => {
    //console.log('[TikTok:roomUser] linea 200:', JSON.stringify(data, null, 2));
    if (data.total) {
